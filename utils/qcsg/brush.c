...
void AddHullEdge( expand_t *ex, vec3_t p1, vec3_t p2 )
{
	...
	for ( i = 0; i < ex->num_hull_edges; i++ )
		if ( ( ex->hull_edges[i][0] == pt1 && ex->hull_edges[i][1] == pt2 ) || ( ex->hull_edges[i][0] == pt2 && ex->hull_edges[i][1] == pt1 ) )
...
restart:
	...
	for ( i = 0; i < 3; i++ )
	{
		if ( h->mins[i] < -BOGUS_RANGE / 2 || h->maxs[i] > BOGUS_RANGE / 2 )
		{
...
void CreateBrush( int brushnum )
{
	...
	// the clipping hull
	if ( contents == CONTENTS_LAVA || contents == CONTENTS_SLIME || contents == CONTENTS_WATER || contents == CONTENTS_TRANSLUCENT )
		return;
