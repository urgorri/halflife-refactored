...
void GetPhongNormal( int facenum, vec3_t spot, vec3_t phongnormal )
{
	...
	if ( smoothing_threshold != 0 )
	{
		...
		for ( j = 0; j < f->numedges; j++ )
		{
			...
			if ( es->coplanar && es1->coplanar && es2->coplanar || VectorCompare( es->interface_normal, vec3_origin ) && VectorCompare( es1->interface_normal, vec3_origin ) && VectorCompare( es2->interface_normal, vec3_origin ) )
				continue;
		...
		if ( extra )
		{
			...
			for ( t = -1; t <= 1; t++ )
			{
				for ( s = -1; s <= 1; s++ )
				{
					...
					if ( ( 0 <= s + sample_s ) && ( s + sample_s < lightmapwidth ) && ( 0 <= t + sample_t ) && ( t + sample_t < lightmapheight ) )
					{
